...
async function checkLazoAccess(lazoId, userId) {
    const result = await database_1.db.query(`SELECT id FROM lazos
     WHERE id = $1 AND (user1_id = $2 OR user2_id = $2) AND is_active = TRUE`, [lazoId, userId]);
...
async function getMessages(req, res) {
    try {
       ...
       LIMIT $2 OFFSET $3`, [lazoId, limit, offset]);
...
async function sendMessage(req, res) {
    try {
        ...
        if (reply_to_id) {
            const replyCheck = await database_1.db.query(`SELECT id FROM messages WHERE id = $1 AND lazo_id = $2`, [reply_to_id, lazoId]);
            if (replyCheck.rows.length === 0) {
                res.status(400).json({ message: 'Mensaje de respuesta no válido' });
                return;
            }
        }
        const result = await database_1.db.query(`INSERT INTO messages (lazo_id, sender_id, content, type, status, reply_to_id)
       VALUES ($1, $2, $3, 'text', 'sent', $4)
       RETURNING id, lazo_id, sender_id, content, type, status, created_at, reply_to_id`, [lazoId, userId, content.trim(), reply_to_id ?? null]);
...
async function sendMediaMessage(req, res) {
    ...
    try {
        ...
        if (replyToId) {
            const replyCheck = await database_1.db.query(`SELECT id FROM messages WHERE id = $1 AND lazo_id = $2`, [replyToId, lazoId]);
            if (replyCheck.rows.length === 0) {
                cleanupFile();
                res.status(400).json({ message: 'Mensaje de respuesta no válido' });
                return;
        ...
        const type = (0, upload_1.isVideoMime)(file.mimetype) ? 'video' : (0, upload_1.isImageMime)(file.mimetype) ? 'photo' : null;
        if (!type) {
            cleanupFile();
